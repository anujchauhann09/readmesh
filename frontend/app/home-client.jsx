...
export default function HomeClient() {
  ...
  // Coalesces the draft write instead of hitting synchronous localStorage on every
  // keystroke. It debounces `source` directly rather than reusing the preview value,
  // which lags a frame behind and would briefly persist an empty draft on load.
  useEffect(() => {
    if (!ready || typeof window === 'undefined') return undefined;
    const timer = setTimeout(() => window.localStorage.setItem(DRAFT_KEY, source), DRAFT_SAVE_DELAY);
    return () => clearTimeout(timer);
  }, [source, ready]);

  // Same helper the server uses to name a saved document, so the title shown here
  // is the title the document ends up with.
